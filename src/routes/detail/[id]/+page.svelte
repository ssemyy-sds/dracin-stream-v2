<script lang="ts">
  ...
  import { Star, Heart, Play, Clock, Calendar, Tag, Loader2 } from 'lucide-svelte';
  ...
  let isFavorited = $derived(drama ? $favorites.some(f => f.bookId === drama.bookId) : false);
      ...
      <div class="relative max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-16">
        <div class="flex flex-col md:flex-row gap-6 md:gap-10">
          ...
          <div class="flex-1 text-center md:text-left">
            <h1 class="text-3xl md:text-4xl font-bold mb-4">{drama.bookName}</h1>
            
            <!-- Meta -->
            <div class="flex flex-wrap items-center justify-center md:justify-start gap-4 mb-6 text-sm text-gray-300">
              {#if drama.rating}
              ...
              <span class="px-2 py-1 rounded-md {drama.status === 'Ongoing' ? 'bg-green-500/20 text-green-400' : 'bg-brand-orange/20 text-brand-orange'}">
                {drama.status}
              </span>
            </div>
            ...
            {#if drama.genres && drama.genres.length > 0}
              <div class="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-6">
                {#each drama.genres.slice(0, 5) as genre}
                  <span class="px-3 py-1 glass rounded-full text-xs">{genre}</span>
                {/each}
              </div>
            {/if}
            ...
            <div class="flex flex-wrap items-center justify-center md:justify-start gap-3">
              ...
              <button
                ...
                class="inline-flex items-center gap-2 px-6 py-3 glass hover:bg-white/20 rounded-full font-semibold transition-colors {isFavorited ? 'text-red-500' : ''}"
              >
