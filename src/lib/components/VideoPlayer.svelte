<script lang="ts">
  ...
  import { Play, Pause, Volume2, VolumeX, Maximize, Loader2, Settings } from 'lucide-svelte';
...
>
  <video
    ...
    playsinline
    onclick={togglePlay}
    onplay={() => { isPlaying = true; isLoading = false; }}
  ...
  {#if isError}
    <div class="absolute inset-0 flex flex-col items-center justify-center bg-black/80">
      ...
      <button
        onclick={() => { isError = false; isLoading = true; updateSource(src); }}
  ...
  {#if !isPlaying && !isLoading && !isError}
    ...
    >
      <div class="w-20 h-20 rounded-full bg-brand-orange/90 flex items-center justify-center shadow-2xl">
  ...
  <!-- Controls -->
  <div 
    class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-4 transition-opacity duration-300 {showControls || !isPlaying ? 'opacity-100' : 'opacity-0'}"
  >
    <!-- Progress Bar -->
    <div 
      ...
      <div class="flex items-center gap-3">
        ...
        {#if qualityOptions.length > 1}
          <div class="relative">
            ...
            {#if showQualityMenu}
              <div class="absolute bottom-full right-0 mb-2 bg-brand-dark rounded-lg overflow-hidden shadow-xl">
                {#each qualityOptions as option}
                  <button
                    onclick={() => selectQuality(option.quality)}
                    class="block w-full px-4 py-2 text-sm text-left hover:bg-white/10 {currentQuality === option.quality ? 'text-brand-orange' : ''}"
                  >
                    {option.quality}p
                  </button>
                {/each}
        ...
        <!-- Fullscreen -->
        <button onclick={toggleFullscreen} class="p-2 hover:bg-white/20 rounded-full transition-colors">
