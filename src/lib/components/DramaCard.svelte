...
>
  ...
  <div class="relative aspect-[2/3] overflow-hidden">
    ...
    <!-- Status Badge -->
    {#if drama.status || drama.cornerLabel}
      <div class="absolute top-2 left-2">
        <span class="px-2 py-1 text-xs font-semibold rounded-md {drama.status === 'Ongoing' ? 'bg-green-500/90' : 'bg-brand-orange/90'} backdrop-blur-sm">
          {drama.cornerLabel || drama.status}
        </span>
      </div>
    {/if}
    ...
    {#if showFavorite}
      <button
        ...
        class="absolute top-2 right-2 p-2 rounded-full glass opacity-0 group-hover:opacity-100 transition-all duration-300 hover:scale-110 {isFavorited ? 'text-red-500' : 'text-white'}"
    ...
    <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
      <div class="w-14 h-14 rounded-full bg-brand-orange/90 flex items-center justify-center backdrop-blur-sm shadow-lg transform scale-75 group-hover:scale-100 transition-transform">
    ...
    {#if drama.latestEpisode}
      <div class="absolute bottom-2 right-2 px-2 py-1 rounded-md glass text-xs font-medium">
  ...
  <div class="p-3 space-y-1">
    <h3 class="font-semibold text-sm line-clamp-2 group-hover:text-brand-orange transition-colors">
      {drama.bookName}
    </h3>
